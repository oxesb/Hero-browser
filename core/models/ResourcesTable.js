...
class ResourcesTable extends SqliteTable_1.default {
    ...
    mergeWithExisting(resourceId, existingResource, newResourceDetails, resourceFailedEvent, error) {
        ...
        existingDbRecord.type ?? (existingDbRecord.type = newResourceDetails.type);
        existingResource.type ?? (existingResource.type = newResourceDetails.type);
        existingDbRecord.devtoolsRequestId ?? (existingDbRecord.devtoolsRequestId = resourceFailedEvent.browserRequestId);
        ...
        existingDbRecord.redirectedToUrl ?? (existingDbRecord.redirectedToUrl = resourceFailedEvent.redirectedToUrl);
        existingDbRecord.statusCode ?? (existingDbRecord.statusCode = newResourceDetails.response.statusCode);
        existingDbRecord.statusMessage ?? (existingDbRecord.statusMessage = newResourceDetails.response.statusMessage);
        existingDbRecord.browserLoadFailure = newResourceDetails.response.browserLoadFailure;
        existingDbRecord.browserLoadedTimestamp ?? (existingDbRecord.browserLoadedTimestamp = newResourceDetails.response.timestamp);
        existingDbRecord.frameId ?? (existingDbRecord.frameId = newResourceDetails.frameId);
