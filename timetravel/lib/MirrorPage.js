...
class MirrorPage extends eventUtils_1.TypedEventEmitter {
    constructor(network, domRecording, showChromeInteractions = false, debugLogging = false) {
        super();
    ...
    async attachToPage(page, sessionId, setReady = true) {
        ...
        try {
            ...
            if (page[installedScriptsSymbol]) {
                promises.push(page.mainFrame.evaluate(`window.domReplayer.reset()`, this.useIsolatedContext));
            }
            else {
    ...
    async open(context, sessionId, viewport, onPage) {
        if (this.isReady)
    ...
    async load(newPaintIndex, overlayLabel, afterLoadedCb) {
        ...
        return await this.loadQueue.run(async () => {
            ...
            this.processPendingDomChanges();
            newPaintIndex ?? (newPaintIndex = this.domRecording.paintEvents.length - 1);
    ...
    async getNodeOuterHtml(paintIndex, nodeId, frameElementNodePointerId) {
     ...
     return null;
   })()`, this.useIsolatedContext, { retriesWaitingForLoad: 2 });
            return { url, html };
        });
    }
    async outerHTML() {
    ...
    processPendingDomChanges() {
        ...
        for (const paint of domRecording.paintEvents) {
            ...
            if (!existing) {
                needsPaintSort || (needsPaintSort = paint.timestamp < lastPaint?.timestamp);
