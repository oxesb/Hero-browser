...
class DomStateAssertions {
    ...
    iterateSessionAssertionsByFrameId(sessionId) {
        var _a;
        (_a = this.assertsBySessionId)[sessionId] ?? (_a[sessionId] = {});
    ...
    recordAssertion(sessionId, frameId, assert) {
        var _a, _b;
        (_a = this.assertsBySessionId)[sessionId] ?? (_a[sessionId] = {});
        (_b = this.assertsBySessionId[sessionId])[frameId] ?? (_b[frameId] = {});
        const assertion = assert;
        assertion.key ?? (assertion.key = DomStateAssertions.generateKey(assertion.type, assertion.args));
