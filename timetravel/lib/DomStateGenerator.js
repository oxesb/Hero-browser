...
class DomStateGenerator {
    constructor(id, emulateSessionId) {
        this.id = id;
    ...
    addSession(sessionDb, tabId, loadingRange, timelineRange) {
        ...
        this.sessionsById.set(sessionId, {
            ...
            get db() {
                return SessionDb_1.default.getCached(sessionId, false);
            },
            dbLocation: SessionDb_1.default.databaseDir,
    ...
    import(savedState) {
        ...
        for (const [frameId, type, args, comparison, result] of savedState.assertions) {
            startingAssertions[frameId] ?? (startingAssertions[frameId] = {});
        ...
        for (const session of savedState.sessions) {
            ...
            let db;
            try {
                db = SessionDb_1.default.getCached(sessionId, false);
            }
            catch (err) {
                // couldn't load
            }
    ...
    async evaluate() {
        if (this.isEvaluating) {
            this.pendingEvaluate ?? (this.pendingEvaluate = new Resolvable_1.default());
    ...
    async doEvaluate() {
        for (const session of this.sessionsById.values()) {
            ...
            session.domRecording = DomChangesTable_1.default.toDomRecording(domChangeRecords, session.mainFrameIds, session.db.frames.frameDomNodePathsById);
    ...
    async createMirrorPageIfNeeded(session) {
        ...
        const networkInterceptor = MirrorNetwork_1.default.createFromSessionDb(session.db, session.tabId, {
            hasResponse: true,
        ...
        this.browserContext ?? (this.browserContext = MirrorContext_1.default.createFromSessionDb(fromSessionId, false)
    ...
    findLastNavigation(session) {
        let lastNavigation;
        const { tabId, db, loadingRange, sessionId } = session;
        const [, endTime] = loadingRange;
        // going in descending order
        for (const nav of db.frameNavigations.getMostRecentTabNavigations(tabId, session.mainFrameIds)) {
